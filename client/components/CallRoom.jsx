...
const CallRoom = ({ roomid }) => {
  ...
  function addPeerVideoStream(newDiv, newVideo, stream, Newimg) {
    ...
    socket.on("togglevideo", (sid, toggle) => {
      console.log(toggle);
      const toggleVideo = document.getElementById(`video ${sid}`)
      const toggleImage = document.getElementById(`image ${sid}`)
      if (toggle == "pause") {
        toggleVideo.style.display = "none";
        toggleVideo.srcObject = null;
  ...
  function connectToNewUser(pid, stream) {
    ...
    newVideo.setAttribute("id", `video ${pid}`);
    Newimg.setAttribute("id", `image ${pid}`);
      ...
      navigator.mediaDevices
        ...
        .then(function (mediaStream) {
          ...
          peerRef.current.on("call", (call) => {
            ...
            newVideo.setAttribute("autoplay", "true");
            newVideo.setAttribute("id", `video ${call.peer}`);
            Newimg.setAttribute("id", `image ${call.peer}`);
            newVideo.setAttribute("class", `rmVideo`);
            Newimg.style.display = "none";
            newDiv.append(newVideo);
            newDiv.append(Newimg);
